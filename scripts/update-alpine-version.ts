...
/** Latest version of `pkg` that is stable (no prerelease tag) and has been published for at least `minAgeHours`. */
async function getEligibleVersion(pkg: string, minAgeHours: number): Promise<string> {
  ...
  if (eligible.length === 0) throw new Error(`No version of ${pkg} older than ${minAgeHours}h found`);
